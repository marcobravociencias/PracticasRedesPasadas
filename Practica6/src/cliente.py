...
class Bob():
	...
	def __init__(self,usr,ip1,ip2):
		...
		self.chat.boton_callVideo.clicked.connect(self.video)
		#self.stopVideo = 0
	...
	def iniciaLlamada(self):
		try:
			...
			self.llamadaExc=exception(self,"Error en llamda")
			self.llamadaExc.show()
		finally:
	...
	def video(self):
		try:
			...
			self.llamadaExc=exception(self,"Error en VideoLLamada")
			self.llamadaExc.show()
		finally:
			SELF.hilo_video.stopEx()
